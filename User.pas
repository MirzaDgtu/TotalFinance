  ...
  if QDelUser = IDYES then
    begin
      ...
      if DeleteUser.FieldByName('Res').AsInteger = 1 then
        QUser := MessageBox(Handle, PChar('Пользователь переведен в резерв, в связи с наличием созданных им документов!'), PChar('Удаление пользователя'), MB_ICONINFORMATION+MB_OK);
    end
  else
    Exit;
end;
