...
begin
 if CheckDate(BegKassaPicker.Date,  EndKassaPicker.Date) then
    Begin
        TotalSummSet.Active := False;
        TotalSummSet.CommandText := Format(SSQLGetOperCashBox, [FormatDateTime('yyyy-mm-dd', BegKassaPicker.Date), FormatDateTime('yyyy-mm-dd', EndKassaPicker.Date)]);
...
begin
    ...
    KassaDetailBar.Panels[1].Text :=  FormatDateTime('dd.mm.yyyy', BegKassaPicker.Date) + ' г.' + ' - ' + FormatDateTime('dd.mm.yyyy', EndKassaPicker.Date) + ' г.';
...
begin
  if CheckDate(BegKassaPicker.Date, EndKassaPicker.Date) then
    Begin
        ...
        ReestrHeadDoc.CommandText := Format(SSQLGetHeaderDocCashBox, [FormatDateTime('yyyy-mm-dd', DTBegPicker.Date),
                                                                       FormatDateTime('yyyy-mm-dd', DTEndPicker.Date)]);
...
begin
    DTBegPicker.Date := BegKassaPicker.Date;
    DTEndPicker.Date := EndKassaPicker.Date;
end;

function TKassaSumForm.CheckDate(BegDate, EndDate: variant): Boolean;
begin
    if BegKassaPicker.Date <= EndKassaPicker.Date then
      Result := True
    else
      Result := False;
end;
...
begin
       if CheckDate(BegKassaPicker.Date, EndKassaPicker.Date) then
          Begin
              TotalSummSet.Active := False;
              TotalSummSet.CommandText := Format(SSQLGetOperCashBox, [FormatDateTime('yyyy-mm-dd', BegKassaPicker.Date),
...
function TKassaSumForm.GetBegDate: variant;
begin
  if PageControl.ActivePage = TabSh1 then
      Result := BegKassaPicker.Date
  else
      Result := EndKassaPicker.Date;
end;

function TKassaSumForm.GetEndDate: variant;
begin
  if PageControl.ActivePage = TabSh1 then
      Result := DTBegPicker.Date
  else
      Result := DTEndPicker.Date;
end;
